...
class Vote_HTML
{
	...
	function rate_html($id, $score)
	{
		...
		return '<a href="' .$settings['domain']. '/index.php?display=guestbook&amp;rate=' .$id. '&amp;score=' .$score. '">' .$href_text. '</a>';
	}
		...
		else
		{
			...
			switch($settings['guestbook_use_comment_rating'])
			{
				...
				case '10-2' :
					...
					break;
				default : 
					error('switch error, option not available: "'.$settings['guestbook_use_comment_rating'].'"', __FILE__, __LINE__);
					break;
			}

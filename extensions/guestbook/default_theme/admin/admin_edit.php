<div class="openld-adblock openld-adform">
	<h2><span><?php echo GB_ADMINISTRATION; ?></span></h2>
		...
		<div class="adformpanel">
			<table cellspacing="0" summary="">
				...
				<?php while($comments = $db->fetch_assoc($comments_sql_limit)) { ?>
					<tr>
						<td>
							<?php echo ($settings['guestbook_use_comment_titles'] == 'Y') ? 'Title: ' .openld_htmlspecialchars($comments['title']). '<br />' : ''; ?>
							...
							<a href="admin_plugins.php?extension=guestbook&amp;delete_comment=<?php echo $comments['id']; ?>&amp;page=<?php echo $req_page; ?>"><?php echo DELETE_LABEL; ?></a> <?php echo $comments['active'] == 0 ? '<a href="admin_plugins.php?extension=guestbook&amp;accept_comment=' .$comments['id']. '">' .ACCEPT_LABEL. '</a>' : '<a href="admin_plugins.php?extension=guestbook&amp;unaccept_comment=' .$comments['id']. '">' .UNACCEPT_LABEL. '</a>'; ?>
						</td>
					</tr>
					<?php } ?>
				</tbody>

	...
	<table>
		<?php if($settings['sponsor_links_option']=='Y') {
			...
			$sponsor_checked = 'checked="checked"';
				...
				$sponsor_checked = '';
			?>
			<tr><td width="130px"><input type='radio' value='sponsor' name='linktype' <?php echo $sponsor_checked; ?> />
			<?php echo SPONSOR_LINK; ?> - <?php echo $sponsor_price; ?><?php ($hook = get_hook('sponsor_information')) ? eval($hook) : null; ?></td><td></td></tr>
		<?php } if($settings['paid_links_option']=='Y') {
			...
			<tr><td width="130px"><input type='radio' value='paid' name='linktype' <?php echo $paid_checked; ?> />
			<?php echo PAID_LINK; ?> - <?php echo $paid_price; ?></td><td></td></tr>
		<?php } if($settings['reciprocal_links_option']=='Y') {
			...
			?>
			<tr><td width="130px"><input type='radio' value='reciprocal' name='linktype' <?php echo $reciprocal_checked; ?> />
			<?php echo RECIPROCAL_LINK; ?></td><td></td></tr>
		<?php } if($settings['regular_links_option']=='Y') { ?>
			<tr><td width="130px"><input type='radio' value='regular' name='linktype' checked="checked" />
			<?php echo REGULAR_LINK; ?></td><td></td></tr>
		<?php } ?>
		<tr>
			<td>
			...
			<td>
				<input type="text" value="<?php echo (isset($_POST['title']) ? openld_htmlspecialchars($_POST['title']) : ""); ?>" name="title" />
				<br />
